...
精简版（Laya 用）用短句代替 JSON：state 约 150 token、题目加选项约 165 token，都在 Laya 英文版的限制内（用局域网 Laya 实测）。

### 目前的局限：这张地图有"必胜解"

把操作交给程序以后，胜负几乎全由计划决定。本地模拟里，固定策略对规则 AI 各打 100 局（公平模式）：

| 固定策略 | 胜率 |
|---|---|
| 一左一右夹击 / 两辆都从左侧进攻 | **100%**（约 12 秒拆掉基地） |
| 左侧进攻 + 一辆守家 | 92% |
| 正面进攻 + 一辆守家 | 72% |
| 两辆都正面进攻 | 56% |
| 追杀坦克 / 守家 / 原地坚守为主 | 0~4% |

基地侧面只隔一层砖，正面要打穿两层，防守几乎拦不住侧面冲锋——谁先冲到对方基地侧面谁就赢。所以在这张地图上，"策略"其实只有一个正确答案，规则 AI 再怎么写也会输给一个无脑冲锋的脚本。要真正测试模型的战术和配合能力，需要先改游戏平衡（加固基地侧面、战争迷雾、占点、道具、车型分工等）。
...
- AI：规则 AI 强于随机 AI；执行层不会打自家基地；战术层只有高层计划；全局视野（位置、区域、移动方向、意图、基地态势）；区域左右对红蓝镜像；三侧进攻路线；反射层闪避与开火；精简版不超出 Laya 的长度预算；远程出错时的兜底
